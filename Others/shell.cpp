...
public:
	// explicit
	// 关键字用来修饰类构造函数，它的作用是禁止隐式转换，避免忽略传参导致错误
	//
	// 「:」提供了一个初始化列表，其在构造函数创建对象时直接对成员变量进行初始化，避免了先定义再赋值的过程，提高了代码的效率和可读性
	// 比如，这里 func(T args) : args(std::move(args)) 中，第一/三个 args
	// 是参数名称，第二个 args 是此类的成员对象名 表示将参数 args
	// 占有的资源转移给成员变量 args
	//
	// override
	// 记号用于显式地指示一个派生类的虚函数覆盖了基类的虚函数，它可以帮助避免在虚函数重写过程中出现一些潜在的错误
	// 比如，如果对一个不是虚函数的函数名使用 override，编译期会报错
	//
	// 由于我们要使用的 syscall 是比较 low-level 的接口，我们需要将 std::string
	// 转换成 char// 才可以被 execvp 函数接收 std::string 类提供 c_str()
	// 接口提供 C 风格的字符串，但是返回值为 const char* C++
	// 中已不推荐使用强制类型转换，取而代之的是 static_cast<T>, dynamic_cast<T>
	// 等 这里的 const_cast<char*>
	// 将一个指向常量对象的指针转换为指向非常量对象的指针
	//
	// 你可以思考：为什么这里的 argv 没有 delete 掉以释放内存？这和 execvp
	// 的运行逻辑密切相关。
	//
	// nullptr 标记参数列表字符串的结束
	//
	// 将参数交给 execvp 函数去执行，如果成功，将执行新的程序；如果失败，将执行
	// cout 和 exit 退出子进程

	explicit ExternalCommand(std::vector<std::string> args)
		: args(std::move(args)) {}
...
public:
	...
	//
	// std::find 函数定义于
	// <algorithm>，可以在一个支持迭代的容器中查找值相等的元素，并返回其迭代器
	// 如果返回 容器.end()，表示 find 没有找到对应的元素

	explicit BuiltInCommand(std::vector<std::string> args)
		: args(std::move(args)) {}
	static bool is_builtin(const std::string &command) {
		const std::vector<std::string> builtin_cmd{"cd", "exit", "pwd", "wait"};
		return std::find(builtin_cmd.begin(), builtin_cmd.end(), command) !=
			   builtin_cmd.end();
	}
	void execute() override {
		if (args[0] == "cd") {
...
public:
	...
	// 考虑到 > 和 >> 的区别，还需要指定是否追加
	// 这可以通过调用 open 时传的参数来做到，O_TRUNC
	// 表示打开文件时清除原来的内容，O_APPEND 则顾名思义
	//
	// 0644
	// 是文件权限设置，它表示拥有者(用户)有读写权限、拥有者组和其他用户有读权限
	//
	// 以输出重定向为例，首先我们打开需要重定向到的文件，获得其文件描述符 fd
	// 然后使用 dup2，把标准输出代表的文件描述符指向的文件重定向至 fd
	// 此时标准输出和 fd 都指向刚刚打开的文件，为了防止出现问题，我们关闭 fd
	// 这个文件描述符（不影响打开的标准输出）
	//
	// 在执行了重定向之后，就可以把剩下的命令部分「递归」地执行，只需要调用其
	// execute 方法即可 注意，这里的 command
	// 是指向抽象类的指针，所以并不需要管它是 buildin or external or
	// others，这就是类型继承的好处

	explicit RedirectCommand(std::unique_ptr<Command> cmd, std::string out_file,
							 std::string in_file, bool append)
		: cmd(std::move(cmd)), out_file(std::move(out_file)),
		  in_file(std::move(in_file)), append(append) {}
	void execute() override {
		if (!out_file.empty()) {
			auto fd =
				open(out_file.c_str(),
...
public:
	// 因为我们要求管道的各部分是并发执行，不能阻塞（如 sleep 3 | echo osh | cat
	// 并不会在三秒之后才输出 osh）
	// 所以管道的各部分都必须是单独在一个进程中执行的
	//
	// 我们将一个管道命令从左往右处理，左边 cmd1 是一个不含管道的语句，右边 cmd2
	// 是余下的部分，可能含有管道 我们 fork
	// 一个子进程，在子进程中将标准输出重定向到管道的写端，接下来关闭管道读端（用不到）、关闭管道写端（已重定向）
	// 接下来，在子进程中执行 cmd1 即可
	//
	// 在当前进程中，继续将剩下的 cmd2 作为一个新的（可能含有管道的）命令来执行
	// 在执行之前，因为 cmd2
	// 是后部的命令，我们需要将其标准输入重定向到管道的读端
	//
	...
	explicit PipeCommand(std::unique_ptr<Command> cmd1,
		...
		: cmd1(std::move(cmd1)), cmd2(std::move(cmd2)) {}
...
public:
	// shell command 是我们的 shell
	// 的最上层的命令类型，这一层额外做的判断是其是否是后台命令 由于我们解析
	// shell command 时，因为只需要考虑命令行末尾有一个
	// &，不需要考虑嵌套，所以我们使用一个 background_pid 作为记录
	//
	// 我们一开始屏蔽了 SIGTTOU 信号，子进程执行时需要重新恢复默认信号处理函数
	// 注：exec 会自动将设置为非 SIG_DFL 且非 SIG_IGN 的信号处理函数重置为
	// SIG_DFL 因为信号处理函数是在原地址空间的，exec
	// 后是无法调用自定义的信号处理函数的

	explicit ShellCommand(std::unique_ptr<Command> cmd, bool is_bg)
		: cmd(std::move(cmd)), is_bg(is_bg) {}
	void execute() override {
		// 首先需要看这个命令是否是内建命令，内建命令不会 fork
		// 新的进程，也就无所谓后台运行
		auto is_built_in = dynamic_cast<BuiltInCommand *>(cmd.get()) != nullptr;

		if (is_built_in) return cmd->execute();
		...
		if (!is_bg)	   // 如果是前台进程
			tcsetpgrp(0, getpid()); // 将终端的前台 pgid 设置为当前进程的
									// pid，确保前台进程可以接收终端的 IO
...
class CommandParser {
public:
	// istringstream 是一个输入的字符串流，它的作用类似于 C 中 sscanf,
	// sprintf，但功能更为强大 如 iss >> word，表示从字符串流中读取一个 token 到
	// word 中
	...
	static std::unique_ptr<Command> parseRedir(std::vector<std::string> words) {
		...
		if (cmd_idx != std::numeric_limits<unsigned>::max()) {
			...
			auto baseCommand = parseBase(std::vector<std::string>{
				words.begin(), words.begin() + cmd_idx});
			return std::make_unique<RedirectCommand>(
				std::move(baseCommand), std::move(out_file), std::move(in_file),
				append);
		}
	...
	static std::unique_ptr<Command> parsePipe(std::vector<std::string> words) {
		...
		if (it == words.end()) { return parseRedir(std::move(words)); }
		...
		if (!left_command || !right_command) return nullptr;

		return std::make_unique<PipeCommand>(std::move(left_command),
											 std::move(right_command));
	}
	// 终端命令
	static std::unique_ptr<ShellCommand> parse(const std::string &line) {
		...
		if (words.empty()) return nullptr;
		auto bg = words.back() == "&";
		if (bg) words.pop_back(); // 记录信号，将 & 从 words 中删除
...
// 处理信号
void init_signal() {
	// signal 函数是将「信号」的处理方法设置成第二个参数
	// handler（可以理解成回调函数） handler 需要是函数指针，而不带捕获的 lambda
	// 表达式可以隐式类型转换成函数指针 这里忽略 SIGTTOU 信号；自定义 SIGINT
	// 以实现丢弃命令；自定义 SIGCHLD 以回收后台进程
